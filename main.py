...
for argument in arguments:
    ARGS[argument] = input("{} >>> ".format(argument))


fonction_courante = function.split(":")[1].strip()
# crée la fonction en gérant uniquement les +
for i, element in zip(range(0, len(fonction_courante.split(" "))), fonction_courante.split(" ")):
    if not element in ["+", ]:
        ast_stack.append({"type": "value", "value": element})
